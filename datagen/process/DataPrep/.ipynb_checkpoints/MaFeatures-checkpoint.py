...
class MaEvalIter(object):
    ...
    def next(self):
        try:
            mas = np.array(self.df.iloc[self.index:self.index+self.n_feat]['MAC'].values, dtype='float64')
        except:
            raise StopIteration()
        #print(mas)
        mads = np.array([], dtype='float64')
...
def decompress(path):
    ...
    if path.endswith('.7z'):
        ...
        subprocess.run(['p7zip', '-d', '-c', path], stdout=new_file)
...
class MaFeatures(object):
    ...
    def add_ma(self, ma_path, info_path):
        ...
        ma_fd, ma_path = decompress(ma_path)
